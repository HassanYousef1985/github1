...
def load_test_data():
        #Load the dataset
        data = pd.read_csv('test.csv', encoding="utf8").fillna(' ')
        # preprocesing
        # rename the labels
        data['check-worthy'] = data['check-worthy'].replace(['yes'],1)
        data['check-worthy'] = data['check-worthy'].replace(['no'],0)
...
def main():
    ...
    if choice == "Convolutions Neural Network (CNN)":
        ...
        if GloVe == '100':
            f = drive.CreateFile({'id':'1jmlAgW6hN1Cv9Ikr4acTA-tdR6H5X2lF'})
            # f = open('glove.6B.100d.txt', encoding='utf8')
        else:
            f = drive.CreateFile({'id':'1TdfeRbwalAmd7N__KmYNn1Oj1BEU-dhu'})
